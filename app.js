(() => {
  ...
  const ui = globalThis.NMDAWorkspaceView.build();
  ...
  function renderProcessGuide() {
    globalThis.NMDAWorkspace?.refresh();
  ...
  function setWorkbenchTab(name) {
    if(name!=='batch' && typeof reviewInlineEl!=='undefined' && reviewInlineEl && !reviewInlineEl.hidden){stashCurrentReviewDraft();hideReviewWorkspaceWithoutStash();}
  ...
  previewBodyEl?.addEventListener('change', event => {
    ...
    if(input.dataset.taskEnabled){
      ...
      if(batch.visualDate || String(batchSearchEl?.value||'').trim())scheduleBatchRender({aux:false});
      return;
    }
  ...
  function reviewVisibleTasks() {
    ...
    if(filter!=='all'){
      scoped=tasks.filter(task=>{
        ...
        if(filter==='attention')return taskHasBlockingIssue(task)||visual.key==='decision';
    ...
    if(filter==='pending'||filter==='decision'||filter==='attention')scoped.sort((a,b)=>reviewTaskPriority(a)-reviewTaskPriority(b) || String(a.editKey).localeCompare(String(b.editKey)));
  ...
  function openReviewWorkspace(options = {}) {
    ...
    batch.reviewFilter=options.filter || (options.pendingOnly?'pending':'all');
    renderProcessGuide();
    setWorkbenchTab('batch');
  ...
  function filteredBatchTasks() {
    return (batch.tasks || []).filter(taskMatchesTagFilter).filter(task=>!batch.visualDate || (task.scheduleAt ? task.scheduleAt.slice(0,10) : 'unscheduled')===batch.visualDate);
  }
  ...
  function renderPreview({ aux = true } = {}) {
    ...
    previewBodyEl.innerHTML=matched.slice(0,batch.planningLimit||150).map(task=>{
      ...
      return `<tr data-task-row="${escapeHtml(task.editKey)}" data-status="${task.status}" data-enabled="${task.enabled?'1':'0'}">
        <td><input type="checkbox" aria-label="选择本封邮件" data-task-enabled="${escapeHtml(task.editKey)}" ${task.enabled?'checked':''} ${batch.running||task.policyBlocked||task.status==='running'||task.status==='done'?'disabled':''} title="${escapeHtml(task.policyBlocked?statusLabel(task):'')}"></td>
    ...
    const more=$('v4-load-more');if(more)more.hidden=matched.length<=(batch.planningLimit||150);
    const count=$('v4-visible-count');if(count)count.textContent=`显示 ${Math.min(matched.length,batch.planningLimit||150)} / ${matched.length} 封`;
  ...
  function resetImportWorkspace({ keepStatus = false, invalidate = true, message = '' } = {}) {
    ...
    batch.handoffComplete = false;
    batch.autoAdvancing = false;
    batch.visualDate='';batch.planningLimit=150;
    batch.dataset = null;
  ...
  globalThis.NMDAWorkspace.mount({
    root:ui,
    snapshot:()=>({tasks:batch.tasks,visible:filteredBatchTasks(),hasSource:!!batch.dataset,busy:batch.importBusy,running:batch.running,step:batch.uiStep,handed:batch.handoffComplete,rules:batch.scheduleRules,date:batch.visualDate||'',
      reviewState:reviewVisualState,issue:taskHasBlockingIssue,attachmentCount:attachmentPreparedFileCount(),attachmentIssues:importAttachmentStats().issues,rosterCount:referenceRosterCount()}),
    review:(filter='all',key='')=>{if(!batch.dataset||batch.running)return;openReviewWorkspace({returnStep:batch.uiStep||2,taskKey:key,filter});},
    attachments:()=>{if(!batch.running)openAttachmentManager();},
    schedule:()=>openScheduleModal(),
    forecast:()=>Scheduler.buildPlan(batch.tasks,readScheduleRuleControls(),new Date()),
    setDate:date=>{batch.visualDate=date;batch.planningLimit=150;renderPreview({aux:false});},
    more:()=>{batch.planningLimit=(batch.planningLimit||150)+150;renderPreview({aux:false});},
    render:()=>renderPreview({aux:false}),
    tab:setWorkbenchTab,
    step:goToProcessStep,
    stop:()=>batchStopEl.click(),
    closeTop:()=>{
      if(!$('nmda-attachment-manager-overlay').hidden){closeAttachmentManager();return true;}
      if(!$('nmda-import-editor-overlay').hidden){closeImportTaskEditor();return true;}
      if(!$('nmda-schedule-modal').hidden){closeScheduleModal();return true;}
      if(!$('nmda-contact-modal').hidden){$('nmda-close-contact-modal').click();return true;}
      if(!$('nmda-supplement-preflight').hidden){$('nmda-close-supplement-preflight').click();return true;}
      return false;
    }
  });
